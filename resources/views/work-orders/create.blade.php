...
<div class="row mb-4">
    <div class="col-md-6">
        ...
        @isset($typeName)
            <h5 class="text-muted">{{ $typeName }}</h5>
        @endisset
    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                            ...
                            @foreach(\App\Constants\WorkOrderTypes::TYPES as $slug => $name)
                                <option value="{{ $slug }}" 
                                    {{ old('type', $workOrder->type ?? request('type', '')) == $slug ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('start_date') is-invalid @enderror" 
                               ...
                               value="{{ old('start_date', isset($workOrder) ? (\Carbon\Carbon::parse($workOrder->start_date)->format('Y-m-d')) : date('Y-m-d')) }}" required>
                        @error('start_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('end_date') is-invalid @enderror" 
                               ...
                               value="{{ old('end_date', isset($workOrder) && $workOrder->end_date ? \Carbon\Carbon::parse($workOrder->end_date)->format('Y-m-d') : '') }}">
                        @error('end_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
